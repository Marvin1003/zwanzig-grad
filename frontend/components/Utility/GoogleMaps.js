...
import mapStyle from '../../../static/json/googleMaps/zwanzig_grad.json';
  ...
  animateMap() {
    ...
    this.mapAnimation.to(this.map, duration, {
      bezier: {
        type: 'quadratic',
        values: [
          { x: 0, y: 0 }, { x: radius, y: 0 }, { x: radius, y: radius },
          { x: radius, y: radius * 2 }, { x: 0, y: radius * 2 },
          { x: -radius, y: radius * 2 }, { x: -radius, y: radius },
          { x: -radius, y: 0 }, { x: 0, y: 0 }
        ]
      },
