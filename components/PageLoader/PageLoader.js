import toSpan from "helper/toSpan";
...
export default class extends React.Component {
  ...
  componentDidMount() {
    window.addEventListener("load", this.componentReady);
  ...
  componentReady = () => {
    ...
    this.firstTL.staggerFromTo(
      ".animate_text_zwanzig",
      this.duration[0],
      { alpha: 0, x: this.distance },
      { alpha: 1, x: 0, ease: Power4.easeOut },
      this.cookieMessageStaggerDelay
    );
  };
  ...
  fadeOut = () => {
    ...
    const lastTL = new TimelineLite({
      onComplete: this.props.removeMe,
      force3d: true
    });
    lastTL
      .staggerTo(
        target,
        this.duration[0],
        { opacity: 0, x: -this.distance },
        this.cookieMessageStaggerDelay
      )
      .to(
        this.loader.current,
        this.duration[1],
        { scaleX: 0, ease: "zwanzig-grad" },
        this.duration[0]
      )
      .to(
        this.loader.current.parentNode,
        this.duration[1],
        { scaleX: 0, onComplete: this.props.removeMe, ease: "zwanzig-grad" },
        this.duration[0] + 0.1
      );
    ...
    return (
      <div className="page_transition_wrapper page_transition_initial">
        <div ref={this.loader} className="page_transition didonesque_normal">
          <div ref={this.textZwanzig} className="zwanzig-grad">
            zwanzig grad
          </div>
        </div>
      </div>
