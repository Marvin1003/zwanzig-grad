...
import runOnce from "helper/runOnce";
...
class LazyLoading extends PureComponent {
  ...
  prepare = () => {
    if (this.props.group) {
      this.groupImages = [
        ...this.container.current.querySelectorAll(".lazy_image")
      ];
  ...
  createData(image) {
    const master = `../static/images/${this.props.imgType}/${image}${
      this.props.mime
    }`;
  ...
  applySource(image, src) {
    ...
    if (this.props.imgTag) TweenLite.set(image, { src });
    else TweenLite.set(image, { backgroundImage: `url(${src})` });
  ...
  lazyLoading(src, cookie) {
    ...
    if (this.props.group) this.handleComplete();
  }
  ...
  render() {
        ...
        >
          {this.props.master.map((image, i) => (
            <div
              ...
              key={`${this.props.imgType}${i + 1}`}
            >
              <Tag
                data-name={`${this.props.imgType}${i + 1}`}
    ...
    return (
      ...
      >
        <Tag
          data-name={this.props.imgType + this.props.master}
          ref={this.image}
          alt={this.props.alt}
