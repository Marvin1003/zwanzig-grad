...
import styles from "./SecNextStyles";
...
class SecNext extends React.Component {
  ...
  componentDidMount() {
    ...
    this.tween = TweenLite.to(".sec_next_wrapper", this.duration, {
      height: "50%",
      top: "50%",
      ease: "zwanzig-grad",
      paused: true,
      onComplete: this.handleCompleteSec
    });
  }
  ...
  setScrollHeight = () => {
    this.scrollHeight =
      Math.max(
        ...
        this.alternativeElem.scrollHeight,
        this.alternativeElem.parentNode.scrollHeight
      ) - window.innerHeight;
  };
  ...
  animate = () => {
    ...
    if (!this.hovering && this.ready) {
      ...
      if (window.innerWidth > 1024) {
        tl.set(this.alternativeElem, {
          scrollTo: this.scrollHeight,
          ease: "zwanzig-grad"
        })
          .to(".sec_next_topic > h1", this.duration, {
            yPercent: -110,
            ease: Power4.easeOut
          })
          .to(
        ...
        tl.set(this.alternativeElem.parentNode, {
          scrollTo: this.scrollHeight,
          ease: "zwanzig-grad"
        })
          .to(".sec_next_topic > h1", this.duration, {
          ...
          .set(".sec_next_wrapper", { display: "none" })
          .to(".sec_next_background", this.duration, {
  ...
  hoverReverse = () => {
    if (window.innerWidth > 1024) {
      this.hovering = false;
      if (!this.clicked) this.tween.reverse();
    }
  };
      ...
      <section className="section sec_next mobile_height" data-size={1}>
        ...
        <div className="sec_next_wrapper">
          <div
            ...
            onMouseEnter={this.hoverPlay}
            onMouseLeave={this.hoverReverse}
            className="sec_next_topic"
          >
            <h1 className="quattrocento_normal">{this.props.next}</h1>
          </div>
          <div
            onClick={this.animate}
            onMouseEnter={this.hoverPlay}
            onMouseLeave={this.hoverReverse}
            className="sec_next_image"
          >
            <Lazy
              lazy={false}
              master={5}
