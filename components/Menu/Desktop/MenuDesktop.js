...
import TextHover from "components/Hover/Text/Text";
import Drag from "helper/Drag";

import styles from "./MenuDesktopStyles";
...
class Menu extends React.Component {
  ...
  toggleAnimation = prevProps => {
    if (prevProps.reverse !== this.props.reverse) {
      if (this.props.reverse) this.tl.reverse();
      else this.tl.play();
    }
  };
      ...
      return (
        <li key={elem} onClick={() => this.props.routeHandling(link)}>
          <a className="topic_link_menu">
            <img
              src={`static/images/${link.replace(/ö/, "oe")}/5${
                this.props.mime
              }`}
              alt={`${link} Bild`}
            />
            <div className="quattrocento quattrocento_normal">
              <h1 className="stagger_item">{elem}</h1>
            </div>
          </a>
        </li>
      );
    ...
    return (
      <>
        <nav className="wrapper dragger">
          ...
          <ul ref={this.list}>{this.generateMenu()}</ul>
        </nav>
