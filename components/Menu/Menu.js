...
const Desktop = dynamic(import("./Desktop/MenuDesktop"), { loading: () => null });
const Mobile = dynamic(import("./Mobile/MenuMobile"), { loading: () => null });
...
class Menu extends React.Component {
  ...
  toggleMenu = () => {
    if (this.state.reverse) this.animateMenuIcon("open");
    if (!this.state.reverse) this.animateMenuIcon("close");
  ...
  animateMenuIcon(type) {
    if (type === "open") {
      ...
      var rotation = 0;
      var y = 0;
      if (location.pathname !== "/") var color = "black";
    ...
    TweenLite.fromTo(
      ...
      { rotation, y, ease: Power4.easeOut }
    );
    TweenLite.fromTo(
      this.strokes[1],
      this.duration,
      { transformOrigin: "50% 0%" },
      { rotation: -rotation, y: -y, ease: Power4.easeOut }
    );
  }
  ...
  routeHandling = req => {
    ...
    } else this.props.context.toggleMenu(req);
  };
...
export default props => (
  <RouterContext.Consumer>
    {nextRoute => (
      <DeviceContext>
        {target => (
          <Menu {...props} context={Object.assign({}, nextRoute, target)} />
        )}
      </DeviceContext>
    )}
  </RouterContext.Consumer>
