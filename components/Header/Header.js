...
class Header extends React.Component {
  ...
  componentDidMount() {
    ...
    this.tween = TweenLite.to(this.nav.current, 0.5, {
      alpha: 0,
      display: "none",
      ease: "zwanzig-grad",
      onStart: () => (this.tweening = true),
      onReverseComplete: () => (this.tweening = false),
      paused: true
    });
  ...
  setAlternativeElem = () => {
    if (window.innerWidth > 1025) {
      this.alternativeElem =
        document.querySelector(".parallax") ||
        document.querySelector(".root") ||
        document.querySelector(".layout_wrapper");
    } else {
      this.alternativeElem =
        document.querySelector(".root") ||
        document.querySelector(".layout_wrapper");
  ...
  getScrollTop() {
    return parseInt(
      Math.max(
        ...
        this.alternativeElem.scrollTop,
        -this.alternativeElem.getBoundingClientRect().top,
        0
      )
    );
  }
  ...
  setScrollHeight = () => {
    this.scrollHeight =
      Math.max(document.body.scrollHeight, this.alternativeElem.scrollHeight) -
      window.innerHeight;
  ...
  animateHeader = () => {
    ...
    if (!window.APP.menu) {
      if (
        this.scrollTop > this.prevScrollTop &&
        this.scrollTop >= 0 &&
        this.prevScrollTop >= 0
      )
        this.tween.play();
      else if (this.scrollTop < this.scrollHeight - 5) this.tween.reverse();
    }
  ...
  render() {
    return (
      <nav ref={this.nav} className={`standard_nav ${this.props.header}`}>
        ...
        <div
          ref={this.menuIcon}
          className="menuicon header_item pointer"
