...
export default class ButtonHover extends PureComponent {
  constructor(props) {
    ...
    this.domPreperation = Array.from(this.props.name, (character, i) => (
      <span key={i} style={this.style.wrapperStyle} className="spanWrapper">
        <span className="move" style={{ display: "inline-block" }}>
          {character}
        </span>
        <span className="move" style={{ display: "inline-block" }}>
          {character}
          ...
          onComplete: () => {
            ...
            switch (type) {
              case "in":
                ...
                if (!this.hover.hoverCheck) hoverOut.call(this, "repeat");
                break;
              case "out":
                this.hover.runningTwo = false;
                if (this.hover.hoverCheck) hoverIn.call(this, "repeat");
                break;
            }
          }
        },
  ...
  hoverInAnimation() {
    ...
    TweenLite.to(this.bgbox.current, this.props.duration, {
      scaleX: 1,
      ease: "zwanzig-grad"
    });
