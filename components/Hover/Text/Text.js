...
export default class TextHover extends PureComponent {
  ...
  hoverInAnimation() {
    ...
    hoverIn
      ...
      .to(this.effect.current, this.props.duration, {
        width: "100%",
        ease: "zwanzig-grad"
      });
  }
  ...
  hoverOutAnimation() {
    ...
    const hoverOut = new TimelineLite({
      onComplete: () => {
        ...
        this.hover.hoverCheck ? hoverIn.call(this, "repeat") : null;
      }
    });
    hoverOut
  ...
  render() {
    ...
    if (this.props.link)
      return (
        ...
        >
          <a style={this.style.elem}>{this.props.name}</a>
          <a
            ref={this.effect}
            style={Object.assign({}, this.style.effect, this.props.effectStyle)}
          >
            {this.props.name}
          </a>
        </Tag>
