...
export class RouterProvider extends Component {
  ...
  toggleMenu = req => {
    ...
    if (req === "button") {
      ...
      this.setState(prevState => {
        if (!prevState.menu) return { menu: true };
      });
      this.nextRoute(req);
    }
  };
  ...
  fadeIn = next => {
    ...
    const handleComplete = () => {
      this.setState(prevState => {
        if (prevState.menu) return { menu: false };
      });
      Router.push(`/${next}`);
    ...
    if (this.allowTransition) {
      ...
      if (this.nextAnimation) {
        ...
        TweenLite.fromTo(
          this.pageTransition.parentNode,
          1,
  ...
  fadeOut() {
    const handleComplete = () => {
      this.nextAnimation &&
        TweenLite.set(this.pageTransition.parentNode, { alpha: 0 });
    ...
    if (this.nextAnimation) {
      ...
      TweenLite.fromTo(
        ...
        1,
        { alpha: 0 },
        { alpha: 1, ease: "zwanzig-grad", onComplete: handleComplete }
      );
    } else {
